...
char* executeCommand(cmdStruct c){
	if(!strcmp(c.cmd, "clear"))
		...
		// PARENT
			...
			int bg = c.bg;
			//Should we start etime here or always at the time a command is entered in shell.c?
			if(!bg)
				waitpid(p, &status, 0);
			//Should probably stop etime/call limits here? It should only exit waitpid once the child has finished.
		}
	}
	return "";
}
...
void eTimeEnd()
{
        ...
        long seconds = tvEnd.tv_sec - tvStart.tv_sec;
        long microseconds = 1000000 - tvStart.tv_usec + tvEnd.tv_usec;
